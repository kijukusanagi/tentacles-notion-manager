...
### Scope Check / Break Down an Oversized Ticket

> "Ticket [ID] feels too big — can you break it down into smaller tickets?"

The agent evaluates the ticket's scope: how many tasks it has, their effort estimates, and whether the ticket title describes a single deliverable or multiple. If it's too broad (more than 5 tasks, mostly L/XL effort, or multi-week scope), the agent proposes splitting it into multiple sprint-sized tickets, each with their own tasks. The original ticket can be converted to an internal project or initiative that the new tickets link to.

**When to use:** After a migration or bulk import, or anytime you realize a ticket is really a project in disguise. Also useful as a periodic hygiene check — ask "are any of my tickets too broad?" and the agent will scan and flag oversized ones.

### Ticket Granularity Audit

> "Audit my tickets — are any of them scoped too broadly?"

The agent reviews all open tickets, checks task counts, effort estimates, and title specificity. It flags any ticket that looks project-sized rather than sprint-sized, and proposes a decomposition plan for each one. You approve which ones to split.
