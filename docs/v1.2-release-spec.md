...
> **Historical design spec (v1.2, March 2026).** The feature descriptions are still accurate, but the **schema and config blocks below are superseded**: the Tickets `Type` enum now includes `Dive` (v1.3), and `Hours Spent` / `Hours Estimated` live under `databases.tasks.other_properties`, not `optional_fields`. The authoritative shapes are `agent/config-template.json` and the Migration Registry in `agent/system-prompt.md`. Do not upload this file to a Claude Project — it is a repo doc, not part of the operating kit.
...
The agent runs a configurable set of "health checks" that scan across all configured databases for problems, risks, and opportunities. Checks run automatically during morning briefings or on demand. Critical findings can auto-generate tickets.
